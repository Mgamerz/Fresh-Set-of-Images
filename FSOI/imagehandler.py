...
class imagehandler:
    ...
    def get_images(self,displaybundle):     
        ...
        threading.Thread(target=lambda: self.threadGetImages(displaybundle)).start()
    ...
    def threadGetImages(self,displaybundle):
        ...
        print('Getting images from '+str(self.sourcequeue.qsize())+' sources.')
        while self.sourcequeue.qsize():
            try:
                ...
                try:
                    ...
                    print('Error getting images from',sourceinfo.pluginid,':',error)
                if newimages:
        ...
        #Check to make sure images in the queue are not already on the filesystem. If they are, remove them.
    ...
    def threadDownloadImages(self):
        ...
        while self.downloadqueue.qsize():
            try:
                ...
                urllib.request.urlretrieve (fileinfo[0], self.parent.download_location.get()+fileinfo[1])
                taskscompleted+=1
    ...
    def getTreeSources(self):
        ...
        sourcesinfo=[] #This list contains the information that will be injected into the tree.
        ...
        for plugin in plugins:
            if plugin[self.PLUGINID]!='undefined':
                ...
                print('Plugin',plugin[0],'does not have the id set, ignoring.')
        return sourcesinfo
        ...
        for root, dirs, files in os.walk(pluginpath):
            ...
            if candidates:
                for c in candidates:
                    ...
                    loader = importlib.machinery.SourceFileLoader(modname, root+os.sep+c)
                    module=loader.load_module(modname)
                    
                    #module=__import__(root+'.'+modname, None, None, "*")   #<-- You can get the module this way
                    for cls in dir(module):          #<-- Loop over all objects in the module's namespace
                        ...
                        and issubclass(cls,pluginbase)): #Make sure it is a subclass of base
                            ...
                            try:
                                ...
                                print(c,'is not fully defined (or type error occured), cannot fully load plugin:',error)
                                continue
                            if not self.load_plugin_dependencies(plugin):
                                classid_map.append((cls.pluginid,False,plugin)) #add it to the list of plugins. It was able to load, but we won't enable it, and the user won't be able to eitehr.
                                continue
                            plugin.load_plugin() #let the plugin loadup
                            classid_map.append((cls.pluginid,True,plugin)) #initialize a single object of it. We shouldn't waste memory making many more of the same objects.
        return classid_map
    ...
    def load_plugin_dependencies(self, plugin):
        ...
        for dependency in dependencies:
            try:
                ...
                print(plugin.pluginid, 'has unmet dependency:', dependency,', attempting to load from directory.')
                try:
...
if __name__=='__main__':
    #main method
    print('ERROR: This file cannot be run on its own. You must use freshsetofimages.py instead.')
